...
function isBranchTypeMatch(save_branch_type, branch_type) {
  if (branch_type === 'branch' && !save_branch_type) {
    return true;
  }
  if (!branch_type && !save_branch_type) {
    return true;
  }
  if (!branch_type && save_branch_type === 'branch') {
    return true;
  }
  return save_branch_type === branch_type;
}

function findBranch(branches, name, branch_type) {
  var foundBranch = false;
  branches.some(function (branch) {
    if (branch.name) {
      var regEx = new RegExp(`^${branch.name.replace(/\*/g, '.*')}$`);
      if (regEx.test(name) && isBranchTypeMatch(branch.branch_type, branch_type)) {
        foundBranch = branch;
        return true;
      }
    }
